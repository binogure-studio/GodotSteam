...
void Steam::OnUploadScore(LeaderboardScoreUploaded_t *callData, bool bIOFailure){
...
void Steam::OnLeaderboardEntriesLoaded(LeaderboardScoresDownloaded_t *callData, bool bIOFailure){
  // Incorrect leaderboard
  if(callData->m_hSteamLeaderboard != leaderboardHandle){
    return;
  }
...
void Steam::findLeaderboard(const String& name){
  ...
  callResultFindLeaderboard.Set(apiCall, this, &Steam::OnFindLeaderboard);
}
...
void Steam::downloadLeaderboardEntries(int rStart, int rEnd, int type){
  ...
  SteamAPICall_t apiCall = SteamUserStats()->DownloadLeaderboardEntries(leaderboardHandle, ELeaderboardDataRequest(type), rStart, rEnd);

  // Register for the async callback
  callResultEntries.Set(apiCall, this, &Steam::OnLeaderboardEntriesLoaded);
}
...
void Steam::uploadLeaderboardScore(int score, bool keepBest){
  ...
  SteamAPICall_t apiCall = SteamUserStats()->UploadLeaderboardScore(leaderboardHandle, ELeaderboardUploadScoreMethod(method), (int32)score, NULL, 0);
  callResultUploadScore.Set(apiCall, this, &Steam::OnUploadScore);
}
...
void Steam::updateLeaderboardHandle(SteamLeaderboard_t lHandle){
  ...
  is_valid = false;
  if(leaderboardHandle > 0 && SteamUserStats() != NULL){
    is_valid = true;
  }
}
